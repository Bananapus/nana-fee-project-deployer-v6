...
**Variations:**
- Deployer-only repos (no `src/`, only `script/`) may skip slither entirely if there are no runtime contracts to analyze.
...
- Published versions use exact npm pins; avoid semver ranges and `latest`
