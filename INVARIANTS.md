...
- **A.2.4 Cross-chain auto-issuance is fixed at four entries.** Always exactly four `REVAutoIssuance` rows (`script/Deploy.s.sol:124–138`), one per chain in the supported set (Ethereum / Base / Optimism / Arbitrum), each beneficiary is `operator`, amounts hardcoded:
  ...
  Every chain in a set writes the *same* four entries so the encoded-configuration hash matches across chains (`script/Deploy.s.sol:119–123, 256–302`). This is load-bearing for `REVDeployer.deployFor`'s cross-chain configuration check.
...
- **B.1.2 Canonical match checks nine independent properties** (`script/Deploy.s.sol:228–253`):
  1. `core.projects.ownerOf(1) == address(revnet.owner)` — the project NFT is held by the `REVOwner` contract, the project's authoritative owner after deployment. `REVDeployer.deployFor` forwards the NFT to `REVOwner` at the end of the deploy, so a re-run against an already-deployed fee project recognizes this ownership as canonical and no-ops idempotently.
  ...
  6. `core.tokens.tokenOf(1).symbol() == "NANA"` (via low-level staticcall + decode at `script/Deploy.s.sol:348–356`).
  7. `core.controller.uriOf(1) == PROJECT_URI` — exact byte-equal IPFS pointer match.
  8. `_reservedSplitIsCanonical(1, operator) == true` — exactly one reserved-tokens split, `100%` to operator, `projectId=0`, `preferAddToBalance=false`, `lockedUntil=0`, `hook=address(0)` (`script/Deploy.s.sol:305–327`).
  9. `_nativeTerminalConfigIsCanonical(1) == true` — primary terminal is `core.terminal`, router-terminal-registry is also a terminal, accounting context `{NATIVE_TOKEN, 18 decimals, NATIVE_CURRENCY}` (`script/Deploy.s.sol:330–345`).
- **B.1.3 Encoded-configuration hash is order-deterministic.** `_encodedConfigurationHashOf` (`script/Deploy.s.sol:256–302`) walks `stageConfigurations` in order, then `autoIssuances` in order, and `abi.encode`-folds each non-zero auto-issuance into the running buffer. Stage start-time monotonicity is enforced (`if (i > 0 && effectiveStart <= previousStageStart) return bytes32(0)` returns the zero hash to guarantee mismatch). The exact same hash function lives in `REVDeployer`; this script implements the mirror so it can compare hashes without trusting the live state.
...
- **`REVOwner`** holds NANA's project NFT after `deployFor` completes (B.2.2). This is the canonical-shape property B.1.2 (1): `REVDeployer` forwards the NFT to `REVOwner` at the end of the deploy, and the canonical-replay check compares ownership against `revnet.owner`. `REVOwner` exposes the operator-rotatable surface for project `#1`; `REVDeployer`'s own on-chain surface for project `#1` is `deploySuckersFor` (operator-only via `REVOwner` permission resolution) and the data-hook callbacks routed through `REVOwner`. Neither has a general "transfer NANA NFT to a different controller" path.
...
| A.4.2 (router-terminal registry is also a terminal) | `script/Deploy.s.sol:334–337` |
...
| B.1.2 (nine canonical-shape properties) | `script/Deploy.s.sol:228–253` |
| B.1.3 (encoded-configuration hash) | `script/Deploy.s.sol:256–302` |
| B.1.4 (zero-amount auto-issuance skipped) | `script/Deploy.s.sol:287` |
| B.1.5 (replay no-op early-return) | `script/Deploy.s.sol:212` |
| B.2.1 (just-in-time NFT approve) | `script/Deploy.s.sol:215–216` |
| B.2.2 (NFT forwarded to REVOwner) | `script/Deploy.s.sol:219–225, 240`; `revnet-core-v6/src/REVDeployer.sol` (NFT forwarded to `OWNER`) |
...
| C.1 `_feeProjectIsCanonical` | `script/Deploy.s.sol:228–253` |
| C.1 `_encodedConfigurationHashOf` | `script/Deploy.s.sol:256–302` |
| C.1 `_reservedSplitIsCanonical` | `script/Deploy.s.sol:305–327` |
| C.1 `_nativeTerminalConfigIsCanonical` | `script/Deploy.s.sol:330–345` |
| C.1 `_projectTokenSymbolIs` | `script/Deploy.s.sol:348–356` |
...
| E (REVOwner post-launch NFT custody) | `script/Deploy.s.sol:240`; `revnet-core-v6/src/REVDeployer.sol` (NFT forwarded to `OWNER`) |
